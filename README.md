...
To run the current state of the Hydra and verify hardware load balancing:
...
### 3. Secuencia de Lanzamiento (Comandos de Terminal)

Ahora, abre tu terminal en la carpeta `aegis_proxy` y ejecuta estos comandos uno por uno con precisión:

**Paso A: Inicializar y empaquetar localmente**
```bash
git init
git add .
git commit -m "feat(core): Phase 1 - Silicon pinning, SO_REUSEPORT, and isolated io_uring runtimes"
git branch -M main
